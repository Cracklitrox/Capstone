...
export default function NotificationReadStats({ notificationId, notificationTitle }) {
  ...
  const loadStats = async () => {
    ...
    try {
      ...
      const response = await getNotificationReadStats(token, notificationId);
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gray-50 p-4 rounded-lg space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium text-gray-700">Total de destinatarios:</span>
                <Badge variant="secondary">{stats.totalRecipients}</Badge>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium text-gray-700">Han leído:</span>
                <Badge className="bg-green-100 text-green-700 border-green-300">
                  {stats.readCount} ({getReadPercentage()}%)
                </Badge>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium text-gray-700">No han leído:</span>
