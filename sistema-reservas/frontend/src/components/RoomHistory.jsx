  ...
  useEffect(() => {
    if (token) {
      fetchRooms(token)
        .then(setAllRooms)
        .catch((err) =>
          console.error("Error al cargar habitaciones para filtros:", err)
        );
    }
  }, [token]);
    ...
    if (isEditingObservation) {
      try {
        const observationToSave =
          newObservation.trim() === "" ? "Sin observaciones" : newObservation;

        const response = await fetch(
          `${API_URL}/reservation_history/${selectedReservation.id}/observation`,
          {
            method: "PUT",
            headers: {
    ...
    <main className="container mx-auto p-4 space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Encuentra reservas completadas por diferentes criterios.
          </CardDescription>
        </CardHeader>
        <CardContent>
                      ...
                      <div className="grid grid-cols-2 gap-4 border p-4 rounded-md">
                        ...
                        <p>
                          ...
                          {selectedReservation.paid_amount !== undefined
                            ? selectedReservation.paid_amount.toLocaleString(
                                "es-CL"
                              )
                            : "No disponible"}
                        </p>
                ...
                <TabsContent value="tab2">
                  <div className="space-y-6 max-h-[70vh] overflow-y-auto p-1 pr-4">
                    <section>
                      ...
                      <ul>
                        {selectedReservation.reservation_rooms.map((rr) => (
                          <li key={rr.id}>
                            {rr.rooms.room_number} - {rr.rooms.room_types.name}
                          </li>
                        ))}
                      </ul>
                    </section>
                    <section>
                      ...
                      {selectedReservation
                        .users_reservations_main_guest_idTousers.phone_number ||
                        "No disponible"}
                    </section>
                    ...
                    <section>
                      ...
                      <ul>
                        {selectedReservation.reservation_services.map((rs) => (
                          <li key={rs.id}>
                            {rs.services.name} (x{rs.quantity})
                          </li>
                        ))}
                      </ul>
                    </section>
                    <section>
                      ...
                      {isEditingObservation ? (
                        ...
                        <p>
                          {selectedReservation
                            .users_reservations_main_guest_idTousers
                            .guest_details?.observations || "Sin observaciones"}
                        </p>
                      )}
