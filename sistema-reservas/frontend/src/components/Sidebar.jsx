...
const Sidebar = ({ sidebarOpen, setSidebarOpen }) => {
  ...
  return (
    <>
      <div
        className={cn("fixed inset-0 bg-black/60 z-20 transition-opacity md:hidden", sidebarOpen ? "opacity-100" : "opacity-0 pointer-events-none")}
        onClick={closeSidebar}
        data-testid="sidebar-overlay"
      />
      <aside
        data-testid="sidebar-component"
        className={cn("fixed top-0 left-0 h-full w-64 p-4 z-30 transition-transform duration-300", "bg-card text-card-foreground border-r border-border", sidebarOpen ? "translate-x-0" : "-translate-x-full", "md:static md:translate-x-0 md:h-auto")}
      >
          ...
          <nav className="flex-grow">
            <ul className="space-y-2">
              {navLinks
                ...
                .map((link) => (
                  <li key={link.label}>
                    <NavLink href={link.href} label={link.label} icon={link.icon} onClick={closeSidebar} />
                  </li>
                ))}
            </ul>
          </nav>
          <div className="mt-auto">
            <p className="text-xs text-muted-foreground">© 2025 Hotel Don Teo</p>
          </div>
        </div>
        <li className="py-2">
