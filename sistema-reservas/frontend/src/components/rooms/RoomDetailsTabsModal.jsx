      ...
      <TabsContent value="reservations" className="space-y-3">
        {recentReservations.length === 0 ? (
          ...
          <>
            ...
            {/* Show counter if there are more items */}
            {recentReservations.length > 3 && (
              <Card className="border-2 border-dashed border-muted-foreground/30">
                <CardContent className="p-4 text-center">
                  <p className="text-sm text-muted-foreground">
                    📋 <span className="font-semibold">{recentReservations.length - 3}</span> reservas más en el historial
                  </p>
                </CardContent>
              </Card>
            )}
        ...
        {cleaningRecords.length === 0 ? (
          ...
          <>
            <Table>
              ...
              <TableBody>
                {/* Show only last 3 cleaning records */}
                {cleaningRecords.slice(0, 3).map((record) => (
            ...
            {/* Show counter if there are more items */}
            {cleaningRecords.length > 3 && (
              <Card className="border-2 border-dashed border-muted-foreground/30 mt-3">
                <CardContent className="p-3 text-center">
                  <p className="text-sm text-muted-foreground">
                    🧹 <span className="font-semibold">{cleaningRecords.length - 3}</span> registros más de limpieza
                  </p>
                </CardContent>
              </Card>
            )}
        ...
        {maintenanceTasks.length === 0 ? (
          ...
          <>
            <Table>
              ...
              <TableBody>
                {/* Show only last 3 maintenance tasks */}
            ...
            {maintenanceTasks.length > 3 && (
              <Card className="border-2 border-dashed border-muted-foreground/30 mt-3">
                <CardContent className="p-3 text-center">
                  <p className="text-sm text-muted-foreground">
                    🔧 <span className="font-semibold">{maintenanceTasks.length - 3}</span> tareas más de mantenimiento
                  </p>
                </CardContent>
              </Card>
            )}
