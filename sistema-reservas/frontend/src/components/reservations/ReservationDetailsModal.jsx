...
const ReservationDetailsModal = ({ reservation, isOpen, onClose, onUpdate }) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="w-[95vw] max-w-4xl h-[90vh] max-h-[900px] flex flex-col p-0">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <div className="w-full overflow-x-auto mb-4">
            <TabsList className="inline-flex w-auto min-w-full">
              <TabsTrigger value="general" className="text-xs sm:text-sm flex-shrink-0">General</TabsTrigger>
              <TabsTrigger value="folio" className="text-xs sm:text-sm flex-shrink-0">Folio</TabsTrigger>
              <TabsTrigger value="pagos" className="text-xs sm:text-sm flex-shrink-0">Pagos</TabsTrigger>
              <TabsTrigger value="huespedes" className="text-xs sm:text-sm flex-shrink-0">Huéspedes</TabsTrigger>
              <TabsTrigger value="habitaciones" className="text-xs sm:text-sm flex-shrink-0">Habitaciones</TabsTrigger>
              <TabsTrigger value="servicios" className="text-xs sm:text-sm flex-shrink-0">Servicios</TabsTrigger>
              <TabsTrigger value="acciones" className="text-xs sm:text-sm flex-shrink-0">Acciones</TabsTrigger>
              <TabsTrigger value="historial" className="text-xs sm:text-sm flex-shrink-0">Historial</TabsTrigger>
            </TabsList>
          </div>
            ...
            <div className="bg-muted p-4 rounded-lg">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <p
                    className={`text-xl font-bold ${reservation.paid_amount >= reservation.total_amount
                      ? 'text-green-600'
                      : 'text-red-600'
                      }`}
                  >
                        ...
                        <tr key={payment.id} className="border-t hover:bg-muted/50">
                          ...
                          <td className="p-3 text-center">
                            {payment.status === 'confirmed' && (
                              <span className="inline-flex items-center gap-1 px-2 py-1 bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200 rounded-full text-xs font-medium">
                                <CheckCircle className="w-3 h-3" />
                                Confirmado
                              </span>
                            )}
                            {payment.status === 'pending' && (
                              <span className="inline-flex items-center gap-1 px-2 py-1 bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-200 rounded-full text-xs font-medium">
                                <Clock className="w-3 h-3" />
                                Pendiente
                              </span>
                            )}
                            {payment.status === 'failed' && (
                              <span className="inline-flex items-center gap-1 px-2 py-1 bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200 rounded-full text-xs font-medium">
          ...
          {/* TAB 4: HUÉSPEDES */}
          <TabsContent value="huespedes" className="space-y-4 mt-4">
            <div>
          ...
          {/* TAB 5: HABITACIONES */}
          <TabsContent value="habitaciones" className="space-y-4 mt-4">
            {reservation.reservation_rooms?.map((rr, index) => (
