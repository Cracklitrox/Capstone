...
import RoomUpgradeModal from '../rooms/RoomUpgradeModal';
...
const ReservationActionsPanel = ({ reservation, onActionComplete }) => {
  ...
  // Cargar pagos para verificar su estado (solo para reservas pendientes)
  useEffect(() => {
  ...
  // Verificar si hay pagos pendientes de confirmar
  ...
  const handleStatusChange = async (newStatus, reason = '') => {
    try {
      ...
      if (!response.ok) {
        ...
        // Si el error es de pago insuficiente, abrir modal de pago
        if (errorData.message && errorData.message.includes('Pago insuficiente')) {
          ...
          <div className="space-y-3">
            ...
            {hasPendingPayments ? (
              ...
              <>
                <Button
                  onClick={() => {
                    // Intentar confirmar siempre - el backend validará el pago
                    handleStatusChange('confirmed', 'Reserva confirmada con pagos completos');
                  }}
                  disabled={isLoading || loadingPayments}
                  className="w-full"
                >
  ...
  return (
    <>
      <div className="space-y-4">
        ...
        <div className="bg-muted p-3 rounded-lg border border-border">
          ...
          <div className="flex justify-between text-sm pt-2 border-t border-border mt-2">
            ...
            <span
              className={`font-bold ${pendingAmount > 0 ? 'text-red-600' : 'text-green-600'
                }`}
            >
      ...
      {/* Diálogo de confirmación - Pago insuficiente */}
      <Dialog open={isConfirmDialogOpen} onOpenChange={setIsConfirmDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Pago insuficiente</DialogTitle>
            <DialogDescription>
              No hay pagos confirmados para esta reserva. Se requiere el pago completo del primer día antes de confirmar la reserva.
              <br /><br />
              <strong>Saldo pendiente:</strong> {formatCLP(pendingAmount)}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
